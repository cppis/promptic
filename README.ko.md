...
| 4 | **커맨드 `>> re` / `>> v2`** | `>> re` (직전 entryId를 parentId로 설정) |
| 5 | **커맨드 `>> last N`** | `>> last 2` (N번째 이전 항목 참조, N: 1~10) |
| 6 | **영어 자연어** | `analyze this prompt`, `how good is this`, `review this prompt` |
| 7 | **한글 자연어** | `이 프롬프트 분석해줘`, `이 프롬프트 어때?`, `프롬프트 좀 봐줘` |
| 8 | **암시적 개선 요청** | `이거 좀 부족한데`, `이거 어떻게 개선해?`, `what's wrong with this` |
...
| 커맨드 | 동작 |
|--------|------|
| `>> anz` / `>> 분석` | 점수, 누락 요소, **개선 프롬프트 블록**(바로 복사 가능) 반환 |
| `>> anz+run` / `>> 분석+실행` | 분석 후 개선된 프롬프트를 **즉시 실행** |
| `>> deep` / `>> 정밀분석` | 분석 → 개선 → 재분석 → 비교 (4단계 파이프라인) |
| `>> re` / `>> v2` | 직전 entryId를 parentId로 설정 (버전 체인 연결) |
| `>> last N` | N번째 이전 entryId를 parentId로 설정 (N: 1~10, ID 조회 불필요) |
...
| 프리셋 | 설명 | 인자 |
|--------|------|------|
| `quick-analyze` | 빠른 5축 분석 + 개선 프롬프트 블록 | `prompt` (필수) |
...
| 프리셋 | 설명 | 인자 |
|--------|------|------|
| `code-review` | 코드 리뷰 프롬프트 작성 + 품질 분석 | `language` (필수), `context` (선택) |
...
| 이전 프롬프트 재분석 | `>> re` 또는 `>> v2` |
| N번째 이전 항목 참조 | `>> last N` (예: `>> last 2`) |
